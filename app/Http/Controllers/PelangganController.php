...
class PelangganController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        Pelanggan::create([
            ...
            'tanggal_pembayaran_terakhir' => null,
            'kwh_terakhir' => $request->kwh_terakhir,
            'kwh_bulan_lalu' => 0,
            'block_rumah' => $request->block_rumah, // ✅ Simpan block rumah
            'kategori_tarif' => $request->kategori_tarif, // ✅ Simpan kategori tarif
        ]);
    ...
    {
        $pelanggan = Pelanggan::where('nomor_pelanggan', $nomor_pelanggan)->firstOrFail();
        // Ambil riwayat pembayaran berdasarkan nomor pelanggan
    $pembayarans = Pembayaran::where('nomor_pelanggan', $pelanggan->nomor_pelanggan)
    ...
    public function invoice($nomor_pelanggan)
    {
        ...
        $sudahTerbayar = $pembayaran && $pembayaran->jumlah_dibayar >= $total_tagihan;
    
        return view('invoice.show', compact('pelanggan', 'pembayaran', 'total_pemakaian', 'total_tagihan', 'sudahTerbayar'));
    }
